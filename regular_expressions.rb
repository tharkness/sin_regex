...
def has_sin?(string)
  unless /\b[0-9]{3}-[0-9]{3}-[0-9]{3}\b/.match(string) == nil
    return true
  else
    return false
  end
...
def grab_sin(string)
  match = /\b[0-9]{3}-[0-9]{3}-[0-9]{3}\b/.match(string)
  match[0] if match
end

#byebug
puts "grab_sin returns an SIN if the string has an SIN"
#test = grab_sin("please don't share this: 234-604-142")
...
def grab_all_sins(string)
  ...
  string.split(" ").each do |item|
    match = /\b[0-9]{3}-[0-9]{3}-[0-9]{3}\b/.match(item)
    sins << match[0] if match
  end
  sins
end
...
def hide_all_sins(string)
  string.gsub(/\b[0-9]{3}-[0-9]{3}-/, "XXX-XXX-")
end
...
def format_sins(string)
  string.gsub(/\b(\d{3})\D?(\d{3})\D?(\d{3})\b/, '\1-\2-\3')
end
